...
import { GradeTableToolbar } from '../grade-table-toolbar';
...
export function GradeListView() {
  ...
  const dataFiltered = applyFilter({
    inputData: grades,
    comparator: getComparator(table.grade, table.gradeBy),
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.admin.grade.details(id));
    },
    [router]
  );
...
function applyFilter({ inputData, comparator, filters, dateError }) {
  ...
  stabilizedThis.sort((a, b) => {
    const grade = comparator(a[0], b[0]);
  ...
  if (name) {
    inputData = inputData.filter(
      (grade) =>
        grade.gradeNumber.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        grade.customer.name.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        grade.customer.email.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((grade) => grade.status === status);
  }

  if (!dateError) {
    if (startDate && endDate) {
      inputData = inputData.filter((grade) => fIsBetween(grade.createdAt, startDate, endDate));
    }
  }
