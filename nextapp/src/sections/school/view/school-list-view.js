...
import { _roles, _schoolList, USER_STATUS_OPTIONS } from 'src/_mock';
...
export default function SchoolListView() {
  ...
  const [tableData, setTableData] = useState(_schoolList);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.admin.school.edit(id));
    },
    [router]
  );
                ...
                icon={
                  ...
                  >
                    {['active', 'pending', 'banned', 'rejected'].includes(tab.value)
                      ? tableData.filter((school) => school.status === tab.value).length
                      : tableData.length}
                  </Label>
                }
              />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (school) => school.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
  ...
  if (role.length) {
    inputData = inputData.filter((school) => role.includes(school.role));
  }
